...
{\color{blue}
Dynamic tensor programs are increasingly common in deep learning, where
operators such as GEMM, convolution, normalization, and attention execute under varying sequence
lengths, batch sizes. Existing libraries and compilers provide
...
opportunities underexplored. We propose MAGSAT, a unified framework for memory-access-guided 
fusion and shape-adaptive auto-tuning. MAGSAT analyzes memory-access semantics to identify
fusion regions, maps them to parameterized tiled templates for GEMM,
convolution, and common epilogues, and selects runtime kernel configurations
with a lightweight trait-driven analytical model. This design decouples
shape-invariant fusion decisions from shape-adaptive parameter selection,
allowing MAGSAT to jointly optimize fusion strategy, kernel template, and tile
configuration for dynamic workloads. We evaluate MAGSAT on A100 and H100 GPUs 
across operator-level and end-to-end Transformer/CNN workloads. Results show 
that MAGSAT consistently improves performance over existing baselines, 
achieving up to XX$\times$ speedup for fused operators and XX$\times$ speedup for end-to-end inference.
}
\end{abstract}
