...
%% Title information
\title{Memory-Access-Guided Fusion and Shape-Adaptive Auto-Tuning for Dynamic GPU Operators}
...
\begin{abstract}
Large language models (LLMs) have driven increasing demand for high-performance GPU kernels. 
...
To address these challenges, we propose a unified framework for dynamic GPU operator optimization that combines memory-access-guided fusion and shape-adaptive auto-tuning. For fused operator generation, our framework analyzes memory access behavior to determine whether and how operators should be fused, and maps different fusion decisions to corresponding compilation templates. For runtime optimization, our framework employs a shape-adaptive auto-tuning strategy that integrates an analytical cost model with residual learning to efficiently select high-performance kernel configurations under unseen shapes. In this way, the framework jointly optimizes fusion strategy, kernel template, and tuning parameters for dynamic workloads.
Experimental results show that our approach consistently outperforms static fusion and static tuning baselines, achieving up to x speedup over existing methods.
\end{abstract}
